...
class TestMerchantContracts(base.BaseTestCase):
    ...
    def test_get_contract_list(self):
        ...
        expected_contracts = []
        for i in range(10):
            merchant_contract = self.get_merchant_contract()
    ...
    def test_get_contract_list_filtering(self):
        merchant = self.create_merchant(self._merchant)
        for i in range(3):
            self.create_merchant_contract(self.get_merchant_contract(), merchant.id, active=False)
        for i in range(3):
            self.create_merchant_contract(self.get_merchant_contract(), merchant.id, currency='UAH')
        expected_contracts = []
        for i in range(3):
            merchant_contract = self.get_merchant_contract()
            merchant_contract["id"] = self.create_merchant_contract(merchant_contract, merchant.id).id
        ...
        status, body = self.get('/merchants/%s/contracts?currency=USD&active=true' % merchant.id)
    ...
    def test_get_contract(self):
        ...
        self.assertEqual(status, 200, msg=body.get('error', ''))
        self.assertDictEqual(body, merchant_contract,
                             msg="\nEXPECTED:\n{}\nACTUAL:\n{}".format(prettify(merchant_contract), prettify(body)))
...
class TestBankContracts(base.BaseTestCase):
    ...
    def test_get_contracts_list_empty(self):
        ...
        status, body = self.get('/payment_systems/%s/contracts' % payment_system.paysys_id)
    ...
    def test_get_contract_list(self):
        ...
        expected_contracts = []
        for i in range(10):
            bank_contract = self.get_bank_contract()
    ...
    def test_get_contract_list_filtering(self):
        payment_system = self.create_payment_system(self._payment_system)
        for i in range(3):
            self.create_bank_contract(self.get_bank_contract(), payment_system.paysys_id, active=False)
        for i in range(3):
            self.create_bank_contract(self.get_bank_contract(), payment_system.paysys_id, currency='UAH')
        expected_contracts = []
        for i in range(3):
            bank_contract = self.get_bank_contract()
            bank_contract["id"] = self.create_bank_contract(bank_contract, payment_system.paysys_id).id
        ...
        status, body = self.get('/payment_systems/%s/contracts?currency=USD&active=true' % payment_system.paysys_id)
    ...
    def test_post_contract(self):
        ...
        status, body = self.post('/payment_systems/%s/contracts' % payment_system.paysys_id, bank_contract)
        ...
        status, body = self.get('/paysys_contracts/%s' % bank_contract["id"])
        self.assertEqual(status, 200, msg=body.get('error', ''))
        self.assertDictEqual(body, bank_contract)
    ...
    def test_get_contract(self):
        ...
        self.assertEqual(status, 200, msg=body.get('error', ''))
        self.assertDictEqual(body, bank_contract,
                             msg="\nEXPECTED:\n{}\nACTUAL:\n{}".format(prettify(bank_contract), prettify(body)))
    ...
    def test_put_contract(self):
        ...
        bank_contract_update = {"commission_fixed": "3.5", "active": False}

        status, body = self.put('/paysys_contracts/%s' % contract.id, bank_contract_update)
